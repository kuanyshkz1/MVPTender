...
final tendersProvider = FutureProvider<List<Tender>>((ref) async {
  ...
  final query = {
    "query": "query { trd_buy(limit: 25) { id name_ru number_anno total_sum customer_name_ru customer_bin ref_buy_status_id end_date } }"
  };
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['data'] != null && data['data']['trd_buy'] != null) {
        ...
        return lotsJson.map((json) => Tender.fromJson(json)).toList();
      } else {
    ...
    data: (tenders) {
      ...
      return tenders.where((tender) {
        ...
        if (filters.keywords.isNotEmpty) {
          final matchesKeyword = filters.keywords.any((keyword) =>
        ...
        // Фильтр по цене
        if (tender.price < filters.priceRange.start || tender.price > filters.priceRange.end) {
          return false;
        }

        // Фильтр по типу
        if (filters.selectedType != 'Все' && tender.type != filters.selectedType) {
          return false;
        }

        // Фильтр по датам
        if (filters.startDate != null && tender.endDate.isBefore(filters.startDate!)) {
          return false;
        }
        if (filters.endDate != null && tender.endDate.isAfter(filters.endDate!)) {
          return false;
        }
...
final saveNoteFamilyProvider =
    FutureProvider.family<void, TenderNote>((ref, note) async {
  final repository = ref.watch(notesRepositoryProvider);
...
final deleteNoteFamilyProvider =
    FutureProvider.family<void, String>((ref, tenderNumber) async {
  final repository = ref.watch(notesRepositoryProvider);
